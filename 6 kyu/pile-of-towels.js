...
// On top of that, you receive a sequence of non-negative integers. The first integer describes
// the number of used towels t in the first week, the second integer describes the number of used
// towels t in the second week and so forth.
//
//   My question is: How will my pile of towels look like in the end, if I use t towels every week and place
//   them on top of the the pile according to the rule defined above?
//
//   Notes
...
function sortThePile(pileOfTowels, weeklyUsedTowels) {
  let max = Math.max(...weeklyUsedTowels);
  let usedTowels = pileOfTowels.slice(pileOfTowels.length - max).sort().reverse();
  return pileOfTowels.slice(0, pileOfTowels.length - max).concat(usedTowels);
}
